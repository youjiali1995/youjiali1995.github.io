...
### Channel
因为有多种 `I/O Multiplexing` 的系统调用，所以用 `Channel` 封装事件并保存对应的 `callback`(为了方便使用)传递给 `Poller`，类似 `Redis` 的 `aeFileEvent`：
```cpp
...
`events_` 是 `Channel` 关心的事件，`Poller` 根据这个来设置。`revents_` 是 `Poller` 返回的已就绪的事件，`handleEvent()` 会调用相应的 `callback` 来处理，`Muduo` 的 `callback` 基本都是
`std::function<void()>`，使用 `std::bind` 绑定参数。  
