...
即使考虑的再周全，仍可能有未知的原因导致 `io_submit` 阻塞，所以 AIO 增加了 [no-wait](https://lwn.net/Articles/724198/) 选项，即当 `io_submit` 要阻塞时会返回 `EAGAIN`，此时程序可以用另外的线程调用 `io_submit` 来防止阻塞主线程。no-wait 对 kernel 和文件系统的版本有要求，同样会在 `seastar` 实现中介绍。
